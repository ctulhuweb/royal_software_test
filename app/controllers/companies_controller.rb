class CompaniesController < ApplicationController
  def index
    ...
    @companies = Company.select('company_tags.name, total_tags, contacts_count').from("(#{contacts_subquery}) as total_contacts, (#{tags_subquery}) as company_tags").where("total_contacts.id = company_tags.id")
